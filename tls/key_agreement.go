...
func hashForServerKeyExchange(sigType uint8, hashFunc crypto.Hash, version uint16, slices ...[]byte) []byte {
	...
	if sigType == signatureECDSA || sigType == signatureSM2 {
...
NextCandidate:
	...
	// change the signature algorithm related code position, if the signature algorithm is SM2SIGWithSM3, the curve
	// must be SM2P256v1, compatible with original code
	...
	if signatureAlgorithm == SM2SIGWithSM3 && curveID != SM2P256v1 {
		hasCurve := false
		for _, c := range clientHello.supportedCurves {
			if SM2P256v1 == c {
				hasCurve = true
		...
		if hasCurve {
			...
			return nil, errors.New("tls: client transport sm certificate but not support sm2-p-256")
		}
	}
