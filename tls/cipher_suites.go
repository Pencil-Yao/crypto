...
// macSM3 returns a macFunction for the given protocol version.
func macSM3(version uint16, key []byte) macFunction {
	return tls10MAC{h: hmac.New(newConstantTimeHash(sm3.New), key)}
}
...
func aeadSM4GCM(key, noncePrefix []byte) aead {
	...
	sm, err := sm4.NewCipher(key)
	if err != nil {
		panic(err)
	}
	aead, err := cipher.NewGCM(sm)
	if err != nil {
		panic(err)
	}
